...
//Insert View Table
useEffect(() => {
  fetch('https://euctostaging.com/prolife/api/stock/products')
    .then((response) => response.json())
    .then((data) => {
      setTableData(data.data); // Assuming 'data' contains the actual array of data
      setIsLoading(false);
    })
    .catch((error) => {
      setIsLoading(false);
  ...
  const initDataTable = () => {
    $('#registaion').DataTable({
      ...
      columns: [
        ...
        { data: 'expiry.batch_no',defaultContent: '-' },
        { data: 'expiry.exp_date',defaultContent: '-' },
        { data: 'expiry.pqty',defaultContent: '-' },
        { data: 'expiry.free',defaultContent: '-' },
        { data: 'expiry.tqty',defaultContent: 'Total Qty' },
    ...
    <div>
      ...
      <div className='py-4 px-5'>
        <div style={{border:'1px solid #000', backgroundColor:'#ffff', borderRadius:'10px'}}>
          ...
          <Row>
              <Col>
              <h3 style={{ fontFamily: 'math'}}><BsCartPlusFill size={35} className='mr-2  mb-2'/>Product Batch Wise Stock</h3>
              </Col>
...
export default ProductBatchStock
